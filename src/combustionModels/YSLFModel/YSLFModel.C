...
:
    ...
    //Lambda_solver_(tableSolver(mesh, Lambda_table())),
    ...
    if(this->active())
    {
       ...
       forAll(Y_, i)
       {
		 ...
		 Lambda = solver_.upperBoundsLambda_(y[2], Lambda_);
                 ...
                 //if (useProgressVariable_)   x[0] = min(chiMax, max(chiMin,chiCells[cellI]));
                 if (useProgressVariable_)   x[0] = min(Lambda, LambdaMax);
                 //if (useProgressVariable_)   x[0] = Lambda;
       ...
       forAll(he_.boundaryField(), patchi)   
       {
          ...
          forAll(Y_, i)
          {
		   ...
		   //if (useProgressVariable_)   x[0] = min(chiMax, max(chiMin, pChi[facei]));
                     if (useProgressVariable_)   x[0] = min(Lambda, LambdaMax);
                     //if (useProgressVariable_)   x[0] = Lambda;
                     if (useMixtureFractionVariance_) x[1] = min(Zeta, 0.99);
                     x[2] = pZ[facei];
