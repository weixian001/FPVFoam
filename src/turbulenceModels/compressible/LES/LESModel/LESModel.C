	...
	if (transportVarZ_)
	{
	    ...
	    (
	        (
	            ...
	            + 2.0*rho_*(mu()/(Sc_*rho_)*magSqr(fvc::grad(this->thermo().Z())) + Cchi_*DZt()*varZ_/(2.0*rho_*sqr(delta())))
	        )
	    );
...
void LESModel::correctChi()			//added
{
...
void LESModel::correctSrr()	//added
{
	//Srr_ = this->thermo().Srr();
//	Srr_.correctBoundaryConditions();
//    Info<< "----------> Strain rate mag, delta  = " << sqrt(magSqr(symm(fvc::grad(U_)))) << ", "
//      << sqr(delta()) << endl;
    Info<< "----------> Srr min/max   = " << min(Srr_).value() << ", "
      << max(Srr_).value() << endl;
}
